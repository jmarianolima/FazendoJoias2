document.addEventListener('DOMContentLoaded', async function() {
    ...
    let position = 0;
    const itemWidth = document.querySelector('.produto-card').offsetWidth + 32; // largura + gap
    const itemsPerView = 5;
    
    prevBtn.addEventListener('click', () => {
        position = Math.min(position + itemWidth * itemsPerView, 0);
        carrossel.style.transform = `translateX(${position}px)`;
    });
    
    nextBtn.addEventListener('click', () => {
        const maxPosition = -(Math.floor(carrossel.children.length / itemsPerView) * itemWidth * itemsPerView);
        position = Math.max(position - itemWidth * itemsPerView, maxPosition);
        carrossel.style.transform = `translateX(${position}px)`;
    });
